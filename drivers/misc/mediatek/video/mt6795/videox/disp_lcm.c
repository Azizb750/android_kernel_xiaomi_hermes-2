...
//Demon start
	if(!strcmp("r69006_dsi_cmd_37_sgm3804", plcm_name)) //Truly
	{
		lcd_gamma = 0;
		printk("Display_DLCM 37 lcd_gamma = %d\n",lcd_gamma);
	}
	else if(!strcmp("nt35596_hd1080_vdo_5A_tps65132", plcm_name))//Sharp
	{
		lcd_gamma = 1;
		printk("Display_DLCM 5A lcd_gamma = %d\n",lcd_gamma);
	}
	else if(!strcmp("nt36760_dsi_vdo_E6_sgm3804", plcm_name))//Auo
	{
		lcd_gamma = 2;
		printk("Display_DLCM E6 lcd_gamma = %d\n",lcd_gamma);
	}
	else if(!strcmp("B2_nt36760_dsi_cmd_sgm3804", plcm_name))//BOE
	{
		lcd_gamma = 3;
		printk("Display_DLCM B2 lcd_gamma = %d\n",lcd_gamma);
	}
	else if(!strcmp("B2_nt35596s_dsi_vdo_lp3101", plcm_name))//BOE
	{
		lcd_gamma = 3;
		printk("Display_DLCM B2 lcd_gamma = %d\n",lcd_gamma);
	}
	...
	{
		lcd_gamma = -1;
		printk("Display_DLCM default lcd_gamma = %d\n",lcd_gamma);
	}
//Demon end
	if (_lcm_count() == 0) {
