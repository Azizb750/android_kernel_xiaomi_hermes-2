...
//add for HeadsetLR
static int Audio_AmpL_HP_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol);
static int Audio_AmpR_HP_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol);
static int Audio_AmpL_HP_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol);
static int Audio_AmpR_HP_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol);
...
extern void AudioAMPDevice_SpeakerLouderOpen(void);
extern void AudioAMPDevice_SpeakerLouderClose(void);
...
static void Voice_Amp_Change(bool enable)
{
    if (enable)
    {
        ...
        if (GetDLStatus() == false)
        {
            ...
            Ana_Set_Reg(ZCD_CON3,  0x3 , 0xffff); //Set voice gain as 5dB
        }
    }
    else
    {
...
static void Speaker_Amp_Change(bool enable)
{
    if (enable)
    {
        ...
        Ana_Set_Reg(ZCD_CON1  , 0x489 , 0xffffffff); // Set LOR/LOL gain as 0dB
#ifdef CONFIG_MTK_SPEAKER
...
SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(amp_function), amp_function), //add this line for HeadsetLR
SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(amp_function), amp_function), //add this line for HeadsetLR
SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(amp_function), amp_function), //add this line for HeadsetLR
};
...
SOC_ENUM_EXT("Audio_Amp_Switch_L_HP", Audio_DL_Enum[12], Audio_AmpL_HP_Get, Audio_AmpL_HP_Set),// add this line for HeadsetLR
SOC_ENUM_EXT("Audio_Amp_Switch_R_HP", Audio_DL_Enum[13], Audio_AmpR_HP_Get, Audio_AmpR_HP_Set),// add this line for HeadsetLR
};

// add for HeadsetLR start
static int Audio_AmpL_HP_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol) 
{ 
printk("%s()\n", __func__); 
return 0; 
...
Ana_Set_Reg(AUDBUF_CFG0, 0x0000, 0x0002); 
...
Ana_Set_Reg(AUDBUF_CFG0, 0x0000, 0x0004); 

mutex_unlock(&Ana_Ctrl_Mutex); 
return 0; 
} 
// add for HeadsetLR end
